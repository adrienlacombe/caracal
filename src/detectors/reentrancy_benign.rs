                        ...
                        for call in reentrancy_info.external_calls.iter() {
                                ...
                                reentrancy_info.variables_read_before_calls.get(call)
                            {
                                    ...
                                    reentrancy_info.storage_variables_written.iter()
                                {
                                    ...
                                    // Benign only when the written variable was
                                    // provably NOT read before the call: an
                                    // unknown identity on either side counts as
                                    // a wildcard match (the pair is reported by
                                    // the full reentrancy detector instead).
