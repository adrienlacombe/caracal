...
echo "bump:     release whose bundled cairo is $TAG (and OZ_TAG if OZ has a newer"
echo "bump:     release), run scripts/corpus.sh --bless, and review the summary"
echo "bump:     diff like a snapshot — any detector dropping to 0 there is the"
echo "bump:     'silently neutered detector' alarm. OZ manifests may need new"
...
echo "bump:     summary together ('Upgrade to cairo $TAG')"
echo
if [[ "$CURRENT_TAG" == "$TAG" ]]; then
