...
- **Source locations**: when caracal compiles the source itself (bundled compiler, both the standalone-file and cairo-project flows), the contract class is compiled with `add_statements_code_locations` + `add_functions_debug_info`, and `src/core/source_map.rs` turns those annotations into a per-statement / per-function `SourceMap` carried on `ProgramCompiled` → `CompilationUnit`. The message helpers append locations as ` (path/to/file.cairo:LINE)` — paths are RELATIVE to the analyzed target (portability: snapshots run on three OSes) and locations outside the target (corelib, including the embedded-corelib temp extraction) are dropped. Pre-built artifacts (Scarb flow, `starknet-compile` fallback) have no mapping: findings keep the location-less format — never render empty parentheses or "unknown". Detector-facing API: `CompilationUnit::statement_location(owner, stmt)` / `function_location(name)`; the location is baked into the message string only, `detector::Result` carries no structured location field yet.
