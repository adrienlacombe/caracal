...
- **Inlined functions are not handled correctly** (upstream limitation) — detectors that reason across calls may miss inlined code; don't "fix" individual detectors for this ad hoc.
- Compiler behavior changes across Cairo versions can silently neuter detectors. Precedent: `unused_arguments` is inert on Cairo ≥ 2.6 (see commit `61488ce`). When a detector stops firing after a compiler bump, check codegen changes before assuming the detector is wrong, and document inert detectors in code comments rather than deleting them.
